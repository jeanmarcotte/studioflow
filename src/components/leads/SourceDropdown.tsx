...
export function SourceDropdown({ value, onChange }: SourceDropdownProps) {
  ...
  return (
    <DropdownMenu>
      ...
      <DropdownMenuContent align="start" className="w-56 bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-700 shadow-lg">
        <DropdownMenuItem onClick={() => onChange(null)} className="cursor-pointer">
          <Check className={cn("mr-2 h-4 w-4", !value ? "opacity-100" : "opacity-0")} />
          <span className="text-gray-900 dark:text-slate-100 font-medium">All Sources</span>
        </DropdownMenuItem>
        ...
        {bridalShows.length > 0 && (
          <DropdownMenuGroup>
            <DropdownMenuLabel className="text-gray-500 dark:text-slate-400 text-xs font-medium uppercase tracking-wide">
              Bridal Shows
            </DropdownMenuLabel>
            {bridalShows.map(source => (
              ...
              >
                <Check className={cn("mr-2 h-4 w-4 text-gray-900 dark:text-slate-100", value === source.id ? "opacity-100" : "opacity-0")} />
                <span className="flex-1 truncate text-gray-900 dark:text-slate-100">{getShortName(source.display_name)}</span>
                {source.lead_count > 0 && (
                  <span className="ml-2 text-xs text-gray-500 dark:text-slate-400">{source.lead_count}</span>
                )}
              </DropdownMenuItem>
            ))}
          </DropdownMenuGroup>
          ...
          <>
            ...
            <DropdownMenuGroup>
              ...
              {digital.map(source => (
                ...
                >
                  <Check className={cn("mr-2 h-4 w-4 text-gray-900 dark:text-slate-100", value === source.id ? "opacity-100" : "opacity-0")} />
                  <span className="flex-1 truncate text-gray-900 dark:text-slate-100">{source.display_name}</span>
                  {source.lead_count > 0 && (
                    <span className="ml-2 text-xs text-gray-500 dark:text-slate-400">{source.lead_count}</span>
                  )}
                </DropdownMenuItem>
