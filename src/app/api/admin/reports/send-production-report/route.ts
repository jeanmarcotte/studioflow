    ...
    attentionHtml = `
      <div style="margin-bottom:28px;">
        ...
        <div style="background:#fffbeb;border:1px solid #fde68a;border-radius:8px;overflow:hidden;">
          <div style="background:#fef3c7;padding:10px 14px;border-bottom:1px solid #fde68a;font-size:12px;font-weight:700;color:#92400e;text-transform:uppercase;letter-spacing:0.5px;">Couples Awaiting Photo Order</div>
...
export async function POST() {
  try {
    ...
    if (emailError) {
      console.error('[Send Production Report] Resend error:', emailError)
      return NextResponse.json({ error: emailError.message }, { status: 500 })
    }

    console.log('[Send Production Report] Email sent:', emailResult?.id)
    return NextResponse.json({ success: true, emailId: emailResult?.id })

  } catch (err) {
    console.error('[Send Production Report] Error:', err)
