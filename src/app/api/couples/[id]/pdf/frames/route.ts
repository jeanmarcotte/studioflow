...
export async function GET(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const [{ data: couple }, { data: items }] = await Promise.all([
    ...
    supabase.from('client_extras').select('*').eq('couple_id', id).order('created_at', { ascending: true }),
  ])
  ...
  if (!items || items.length === 0) {
    ...
    for (const item of items) {
      checkNewPage(ctx)
      drawGap(ctx, 4)
      drawText(ctx, `${item.item_type || 'Item'} \u2014 ${item.description || ''}`, { size: 10, bold: true })
      ...
      if (item.status) drawText(ctx, `Status: ${item.status}`, { size: 9, indent: 8 })
      drawGap(ctx, 4)
      drawLine(ctx)
