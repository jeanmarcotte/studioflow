...
export default function FramesAlbumsQuotePage() {
  ...
  // Collage photo selections (for notes)
  const [collageNotes, setCollageNotes] = useState({
    left: '',
    center: '',
    right: ''
  })
        ...
        <div className="bg-white border rounded-lg overflow-hidden">
          ...
          <div className="p-6 space-y-6">
            ...
            >
              ...
              {/* Collage Photo Notes - only show when selecting photos */}
              {items.engagementCollage && (collageNotes.left || collageNotes.center || collageNotes.right) && (
                <div className="mt-3 ml-4 p-3 bg-zinc-50 rounded text-sm">
                  <p className="font-medium mb-1">Selected Photos:</p>
                  <p className="text-muted-foreground">
                    Left: {collageNotes.left || '—'} | Center: {collageNotes.center || '—'} | Right: {collageNotes.right || '—'}
                  </p>
                </div>
              )}
            </div>
            ...
            {/* Collage Photo Selection (hidden section for Jean to fill in) */}
            {items.engagementCollage && (
              <div className="border-t pt-4 mt-6">
                <p className="text-xs text-zinc-400 mb-2">Photo Selection Notes (for editing):</p>
                <div className="grid grid-cols-3 gap-3">
                  <div>
                    <label className="text-xs text-muted-foreground">Left</label>
                    <input
                      type="text"
                      value={collageNotes.left}
                      onChange={(e) => setCollageNotes(prev => ({ ...prev, left: e.target.value }))}
                  ...
                  <div>
                    ...
                    <input
                      type="text"
                      value={collageNotes.center}
                      onChange={(e) => setCollageNotes(prev => ({ ...prev, center: e.target.value }))}
                      placeholder="# - edit notes"
                      className="w-full px-2 py-1 text-sm border rounded"
                    />
                  </div>
                  <div>
                    ...
                    <input
                      type="text"
                      value={collageNotes.right}
                      onChange={(e) => setCollageNotes(prev => ({ ...prev, right: e.target.value }))}
                      placeholder="# - edit notes"
                      className="w-full px-2 py-1 text-sm border rounded"
                    />
                  </div>
