...
export default function ProductionDashboardPage() {
  ...
  const [coupleMap, setCoupleMap] = useState<Map<string, string>>(new Map())
  ...
  useEffect(() => {
    const fetchAll = async () => {
      const [jobsRes, videoRes, productsRes, couplesRes] = await Promise.all([
        ...
        supabase.from('couples').select('id, couple_name'),
      ])
      ...
      ;(couplesRes.data || []).forEach((c: any) => cMap.set(c.id, c.couple_name))
      setCoupleMap(cMap)
  ...
  const orderStats = useMemo(() => {
    ...
    const atLab = orderJobs.filter(j => j.status === 'at_lab').length
    const waiting = orderJobs.filter(j => j.status === 'waiting_approval').length
    return { total, atLab, waiting }
  }, [orderJobs])
  ...
  const waitingStats = useMemo(() => {
    ...
    const daysSum = allWaiting.reduce((sum, j) => {
      if (!j.sent_for_review_date) return sum
      return sum + differenceInDays(new Date(), parseISO(j.sent_for_review_date))
    }, 0)
    const avgDays = count > 0 ? Math.round(daysSum / count) : 0
  ...
  const waitingGrouped = useMemo((): WaitingItem[] => {
    ...
    waitingJobs.forEach(j => {
      ...
      const days = j.sent_for_review_date ? differenceInDays(new Date(), parseISO(j.sent_for_review_date)) : 0
      ...
      if (existing) {
        ...
        grouped.set(j.couple_id, {
          ...
          coupleName: coupleMap.get(j.couple_id) || 'Unknown',
  ...
  // Orders table data
  const orderTableData: OrderRow[] = useMemo(() => {
    return orderJobs
      .map(j => ({
        id: j.id,
        ...
        couple_name: coupleMap.get(j.couple_id) || 'Unknown',
  ...
  const orderColumns: ColumnDef<OrderRow>[] = useMemo(() => [
    ...
    },
    {
      ...
      accessorFn: (row) => {
        if (row.status === 'waiting_approval' && row.sent_for_review_date) {
          return differenceInDays(new Date(), parseISO(row.sent_for_review_date))
        }
        return differenceInDays(new Date(), parseISO(row.created_at))
      },
      cell: ({ row }) => {
        const days = row.original.status === 'waiting_approval' && row.original.sent_for_review_date
          ? differenceInDays(new Date(), parseISO(row.original.sent_for_review_date))
          : differenceInDays(new Date(), parseISO(row.original.created_at))
    ...
    },
    {
      ...
      header: '',
      cell: ({ row }) => (
        <Link href={`/admin/production/couples/${row.original.couple_id}`} className="text-xs text-teal-600 hover:underline">
          Hub
        </Link>
      ),
      enableSorting: false,
    ...
    <div className={`space-y-6 ${nunito.className}`}>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 px-4 md:px-6">
        ...
        <StatCard
          ...
          lines={[
            `${orderStats.atLab} at lab`,
            `${orderStats.waiting} waiting`,
          ]}
        />
        <StatCard
      ...
      <div className="px-4 md:px-6">
        <h2 className={`text-lg font-bold mb-3 ${playfair.className}`}>Client Orders</h2>
