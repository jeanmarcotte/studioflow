...
import { ArrowLeft, Loader2, Plus, X, Trash2, Send, Save } from 'lucide-react'
...
export default function C3ExtrasForm() {
  ...
  useEffect(() => {
    async function fetchData() {
      const [coupleRes, productsRes] = await Promise.all([
        supabase
          .from('couples')
      ...
      if (sendEmail) {
        await fetch('/api/c3/invoice', {
          ...
          body: JSON.stringify({
            ...
            weddingDate: formatWeddingDate(couple.wedding_date),
  ...
  if (!couple) {
    return (
      <div className="fixed inset-0 z-50 flex items-center justify-center" style={{ backgroundColor: BG }}>
        <p style={{ color: MUTED }}>Couple not found</p>
      </div>
    )
  }
          ...
          <div className="flex items-start justify-between mb-2">
            <div>
              <h1 className={playfair.className} style={{ fontSize: 28, fontWeight: 700, lineHeight: 1.2 }}>
                {couple.bride_first_name} & {couple.groom_first_name} — Extras (C3)
              </h1>
        ...
        <div className="fixed inset-0 z-[60] flex items-start justify-center pt-[10vh] px-4" onClick={() => setShowAddPopup(false)}>
          ...
          >
            ...
            <div className="overflow-y-auto flex-1">
              ...
              {/* All other products by category */}
