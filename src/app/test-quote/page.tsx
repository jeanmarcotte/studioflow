...
export default function TestQuotePage() {
  ...
  const handleGenerateTestPdf = async () => {
    ...
    try {
      await generateQuotePdf(TEST_DATA)
      setStatus('Quote PDF generated!')
    } catch (err) {
      const msg = err instanceof Error ? err.message : String(err)
      setStatus(`QUOTE PDF ERROR: ${msg}`)
      console.error('Quote PDF error:', err)
    }
  }

  const handleGenerateContractPdf = async () => {
    setStatus('Generating contract PDF...')
    try {
      await generateQuotePdf({ ...TEST_DATA, contractMode: true })
      setStatus('Contract PDF generated!')
    } catch (err) {
      const msg = err instanceof Error ? err.message : String(err)
      setStatus(`CONTRACT PDF ERROR: ${msg}`)
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col items-center justify-center gap-6">
      ...
      <button
        ...
        className="px-8 py-4 bg-green-700 text-white rounded-lg text-lg font-medium hover:bg-green-800 transition-colors"
      >
        Generate Test Contract PDF (with Terms)
      </button>
      {status && (
        <p className={`text-sm font-mono ${status.includes('ERROR') ? 'text-red-600' : 'text-green-600'}`}>
          {status}
        </p>
      )}
    </div>
