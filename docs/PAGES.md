...
| Route | Page | Reads From | Writes To | Triggers |
|-------|------|-----------|----------|----------|
| `/admin/production/photo` | Photo Editing Dashboard | jobs, couples, product_catalog, video_jobs (Post-Shoot Backup alert only) | jobs.status, jobs.photos_taken, jobs.edited_so_far, jobs.total_proofs, jobs.{raw_file_path,raw_file_count,raw_file_size_gb,backed_up}, video_jobs.{raw_file_path,raw_file_count,raw_file_size_gb,backed_up} (via `/api/production/backup`) | `flip_engagement_milestones` on status change. **WO-1034**: "Ready to Start" lane includes only `not_started` jobs whose `couples.wedding_date <= CURRENT_DATE`; future-wedding `not_started` jobs render in a separate collapsed-by-default "📅 Upcoming" section. |
| `/api/production/backup` (POST) | WO-1032 Post-Shoot Backup API | — | jobs OR video_jobs (`raw_file_path`, `raw_file_count`, `raw_file_size_gb`, `backed_up=true`) | — |
